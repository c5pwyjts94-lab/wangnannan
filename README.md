...
2. 核心运行顺序
为了保证数据流的完整性，请按任务阶段顺序执行脚本：

第一步 (TASK1 数据清洗)：进入 TASK1 目录，运行预处理脚本生成标准化数据集。

第二步 (TASK2 模型训练)：进入 TASK2 目录，运行 逻辑回归.py 或 XGboost建模.py 训练模型并导出 picture-task2/ 中的科研图表。

第三步 (LaTeX 编译)：使用 VS Code 的 LaTeX 插件或 Overleaf 编译 main.tex 得到最终的学术报告 main.pdf。
